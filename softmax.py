...
show_images(X.reshape(18,28,28), 2, 9, title=get_labels(y))
'''
...
def softmax(X):
    X_exp = torch.exp(X)
    partition = X_exp / X_exp.sum(1,keepdim=True)
    return partition
...
def train_epoch_ch3(net,train_iter,loss,updater):
    ...
    return metric[0]/metric[2],metric[1]/metric[2]#损失率 准确率
...
def predict_ch3(net,test_iter,n=6):
    for X,y in test_iter:
        break
    originals = d2l.get_fashion_mnist_labels(y)
    predics = d2l.get_fashion_mnist_labels(net(X).argmax(axis=1))
    titles = [true + '\n' + pred for true,pred in zip(originals,predics)]
    d2l.show_images( X[0:n].reshape((n, 28, 28)), 1, n, titles=titles[0:n])


num_epochs = 10
train_ch3(net,train_iter,test_iter,cross_entropy,num_epochs,updater)
'''predict_ch3(net,train_iter)
plt.show()'''
